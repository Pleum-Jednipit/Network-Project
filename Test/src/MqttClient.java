...
public class MqttClient extends Thread {
    ...
    public void run() {
        ...
        if (splitcommand[0].equals("subscribe")) {
            ...
            System.out.println("Get message from publisher :" + socket.getRemoteSocketAddress());
            System.out.println("message: " + splitcommand[2]);
            for (MqttClient client : clientList) {
                if (client.topic != null && client.topic.equals(splitcommand[1])) {
                    try {
                        ...
                        client.output.writeUTF(splitcommand[2]);
                    } catch (IOException e) {
                    }
                }
            }
